...
for (var key of map.keys()) {
  ...
  for (let servizio of map.get(key)) {
      let copyKey = key;
    $(`#${servizio.replaceAll(" ", "-")}`).click(() => {
      //Se il filtro è già attivo e viene cliccato di nuovo, allora disattivalo
      if (urlParams.getAll(copyKey).includes(servizio.replaceAll(" ", "-"))) {
        let array = urlParams.getAll(copyKey);
        array = array.filter((item) => {
          return item != servizio.replaceAll(" ", "-");
        });
        urlParams.delete(copyKey);
        for (elemento of array) {
          urlParams.append(copyKey, elemento);
        }
        location.search = urlParams;
      }
      //Altrimenti attivalo
      else {
        urlParams.append(copyKey, servizio.replaceAll(" ", "-"));
        location.search = urlParams;
      }
    });
  }
